...
fun <C, T : Number, E> Specification<C, T, E>.multipleOf(hint: HintBuilder<C, T, E>, factor: Number, epsilon: Double): ConstraintBuilder<C, T, E> {
...
fun <C, T : Number> Specification<C, T, String>.multipleOf(factor: Number, epsilon: Double): ConstraintBuilder<C, T, String> =
    multipleOf(stringHint("must be a multiple of '{0}'"), factor, epsilon)

fun <C, T : Number, E> Specification<C, T, E>.maximum(hint: HintBuilder<C, T, E>, maximumInclusive: Number) =
    addConstraint(hint, maximumInclusive) { it.toDouble() <= maximumInclusive.toDouble() }

fun <C, T : Number> Specification<C, T, String>.maximum(maximumInclusive: Number) =
    maximum(stringHint("must be at most '{0}'"), maximumInclusive)

fun <C, T : Number, E> Specification<C, T, E>.exclusiveMaximum(hint: HintBuilder<C, T, E>, maximumExclusive: Number) =
    addConstraint(hint, maximumExclusive) { it.toDouble() < maximumExclusive.toDouble() }

fun <C, T : Number> Specification<C, T, String>.exclusiveMaximum(maximumExclusive: Number) =
    exclusiveMaximum(stringHint("must be less than '{0}'"), maximumExclusive)

fun <C, T : Number, E> Specification<C, T, E>.minimum(hint: HintBuilder<C, T, E>, minimumInclusive: Number) =
    addConstraint(hint, minimumInclusive) { it.toDouble() >= minimumInclusive.toDouble() }

fun <C, T : Number> Specification<C, T, String>.minimum(minimumInclusive: Number) =
    minimum(stringHint("must be at least '{0}'"), minimumInclusive)

fun <C, T : Number, E> Specification<C, T, E>.exclusiveMinimum(hint: HintBuilder<C, T, E>, minimumExclusive: Number) =
    addConstraint(hint, minimumExclusive) { it.toDouble() > minimumExclusive.toDouble() }

fun <C, T : Number> Specification<C, T, String>.exclusiveMinimum(minimumExclusive: Number) =
